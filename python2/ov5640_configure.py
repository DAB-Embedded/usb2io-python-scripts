...
allow_usb2io_config = 1
allow_init_config = 1
allow_720p_config = 0
allow_1080p_config = 1
#---- Application ---------------------------------------------------------
print "===================================="
print "* OV5640 configure"
print "===================================="

local_com_port = sys.argv[1] if (len(sys.argv) > 1) else COM_PORT_ENG

def ov5640_reg_write(ser, reg, value):
	strx = "i2c write 1, 0x3C," + hex((reg >> 8) & 0xFF) + "," + hex(reg & 0xFF) + ","+ hex(value & 0xFF) + "\r\n"
	wro = ser.write(strx)
	time.sleep(0.02)
	rrd = ser.readline()

def ov5640_reg_read(ser, reg):
	strx = "i2c write 1, 0x3C," + hex((reg >> 8) & 0xFF) + "," + hex(reg & 0xFF) + "\r\n"
	wro = ser.write(strx)
	rrd = ser.readline()
	strx = "i2c read 1, 0x3C,1\r\n"
	wro = ser.write(strx)
	rrd = ser.readline()
	print (rrd)
...
if allow_usb2io_config == 1:
    ...
    usb2io_command(ser, "i2c configure 1, 0\r\n")
    # for the setting , 24M Mlck input and 24M Plck output
    print "Set PLL freq"
    usb2io_command(ser, "pll setfreq 24000000\r\n")
    ...
    # read IDs
    #ov5640_reg_read(ser, 0x300A) # should be 0x56
    #ov5640_reg_read(ser, 0x300B) # should be 0x40

#-----------------------------------------------------------------------------------
if allow_init_config == 1:
    print "OV5640 initialize"

    ov5640_reg_write(ser, 0x3103,0x11) #system clock from pad, bit[1]
    ov5640_reg_write(ser, 0x3008,0x82)	
    time.sleep(0.1)
    ...
    0x4740, 0x22 #HSYNC, VSYNC polarity

    ]
    print "Configure initial"
    for i in range(0, len(config_initial) >> 1):
	    print i
	    ov5640_reg_write(ser, config_initial[i * 2], config_initial[i * 2 + 1])
	
#-----------------------------------------------------------------------------------
if allow_720p_config == 1:
    config_rgb565 = [
    0x4300, 0X6F,
    0X501F, 0x01,
    # 1280x800, 15fps
    ...
    #0x5001, 0x0, # SDE on, scale on, UV average off, color matrix on, AWB on
    #0x5602, 0x00,
    #0x5604, 0x00,
    0x3503, 0x00 # AEC/AGC on  
    ]

    print "Configure rgb565"
    for i in range(0, len(config_rgb565) >> 1):
	    print i
	    ov5640_reg_write(ser, config_rgb565[i * 2], config_rgb565[i * 2 + 1])	

#-----------------------------------------------------------------------------------
if allow_1080p_config == 1:
    config_1080p = [
    0x4300, 0X6F, #6F YuV422
    0X501F, 0x01, #01
    # 1280x800, 15fps
    # input clock 24Mhz, PCLK 42Mhz
    0x3035, 0x41, # PLL
...
#    0x4004, 0x02, # BLC line number
    ...
    print "Configure 1080p"
    for i in range(0, len(config_1080p) >> 1):
	    print i
	    ov5640_reg_write(ser, config_1080p[i * 2], config_1080p[i * 2 + 1])	
